    ...
    with conn() as db:
        db.executescript(
            ...
            CREATE TABLE IF NOT EXISTS quote_items (
              ...
              flete_unitario REAL DEFAULT 0,
              costo_unitario_entregado REAL DEFAULT 0,
              created_at TEXT DEFAULT '',
        ...
        if "latitud" not in cols:
            db.execute("ALTER TABLE clients ADD COLUMN latitud REAL")
        if "longitud" not in cols:
            db.execute("ALTER TABLE clients ADD COLUMN longitud REAL")
        fiscal_cols = {
            "tipo_persona_fiscal": "TEXT DEFAULT ''",
            "rfc": "TEXT DEFAULT ''",
            "regimen_fiscal": "TEXT DEFAULT ''",
        ...
        db.execute(
            ...
            INSERT INTO quote_items
              (quote_id, producto, cantidad, precio_unitario, subtotal_material, flete_unitario, costo_unitario_entregado, created_at)
            SELECT q.id, q.producto, q.cantidad, q.precio_unitario,
                   q.cantidad * q.precio_unitario, q.flete_unitario, q.costo_individual, q.created_at
            FROM quotes q
            WHERE COALESCE(q.producto,'')<>''
              AND NOT EXISTS (SELECT 1 FROM quote_items qi WHERE qi.quote_id=q.id)
            """
        )
...
def generate_quote_pdf(client, data, output_dir=None, filename_override=None):
    ...
    items = data.get("items") or [{
        ...
        "subtotal_material": float(data.get("cantidad") or 0) * float(data.get("precio_unitario") or 0),
        "flete_unitario": data.get("flete_unitario") or 0,
        "costo_unitario_entregado": data.get("precio_ofrecido") or 0,
    }]
    product_rows = [["Producto", "Cantidad", "Precio unitario", "Subtotal material"]]
    for item in items:
        product_rows.append([
            item.get("producto") or "-",
            f"{float(item.get('cantidad') or 0):,.0f}",
            money(item.get("precio_unitario")),
            money(item.get("subtotal_material")),
        ])
    product_table = Table(product_rows, colWidths=[245, 75, 100, 100])
    ...
    story.append(Paragraph("<b>Materiales de la carga</b>", styles["Heading3"]))
...
class Handler(BaseHTTPRequestHandler):
        ...
        if path == "/api/quotes":
            ...
            with conn() as db:
                ...
                for quote in payload:
                    quote["items"] = rows_to_dicts(db.execute(
                        "SELECT * FROM quote_items WHERE quote_id=? ORDER BY id", (quote["id"],)
                    ).fetchall())
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/quote":
            ...
            data["items"] = items
            data["producto"] = product_summary
            data["productos_ofrecidos"] = product_summary
            ...
            data["precio_ofrecido"] = costo_individual
            data["total"] = total
                ...
                for item in items:
                    db.execute(
                        ...
                        (
                            ...
                            item["subtotal_material"],
                            item["flete_unitario"],
                            item["costo_unitario_entregado"],
                            now_iso(),
                        ),
                    )
                db.commit()
